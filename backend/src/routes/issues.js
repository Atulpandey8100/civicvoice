...
const CATEGORIES = ['infrastructure', 'safety', 'environment', 'utilities', 'transportation', 'other'];
const STATUSES = ['pending', 'in-progress', 'resolved', 'closed'];
const SORT_FIELDS = ['voteCount', 'aiPriority', 'createdAt', 'updatedAt', 'title', 'status', 'category'];
...
router.get('/', async (req, res) => {
  try {
    ...
    if (!SORT_FIELDS.includes(field)) sort = '-createdAt';
...
router.get('/stats', async (req, res) => {
  try {
    const [totalIssues, byStatus, byCategory, recentIssues] = await Promise.all([
      ...
      Issue.find().sort('-createdAt').limit(6).populate('author', PUBLIC_AUTHOR)
    ]);
    res.json({
      totalIssues,
...
router.get('/my', auth, async (req, res) => {
  try {
    const issues = await Issue.find({ author: req.userId })
      .populate('author', PUBLIC_AUTHOR)
      .sort('-createdAt');
    res.json(issues);
  } catch (err) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const isOfficial = req.user.role === 'official' || req.user.role === 'admin';
      ...
      if (isOfficial && req.body.status) allowedUpdates.status = req.body.status;
      ...
      if (req.files && req.files.length > 0) {
        ...
        const combined = [...existing, ...req.files.map(f => `/uploads/${f.filename}`)];
        const kept = combined.slice(0, 5);
        const dropped = combined.slice(5);
        await Promise.all(
          dropped.map((img) => fs.promises.unlink(path.join(UPLOADS_DIR, path.basename(img.replace('/uploads/', '')))).catch(() => {}))
        );
        allowedUpdates.images = kept;
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      issue.comments.push({ user: req.userId, text: req.body.text.trim() });
      await issue.save();
